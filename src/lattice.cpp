...
Lattice::Lattice()
{
  ...
  if (param_json.find("lattice") == param_json.end() || 
      ...
      param_json["generated_variables"].find("lattice_nu") == param_json["generated_variables"].end()) {
  ...
  nu = param_json["generated_variables"]["lattice_nu"];  
...
Lattice::populate_Nodes()
{
  ...
  // set zou_he_type after creatinng all the nodes
  #pragma omp parallel for
  for(unsigned int index = 0; index<nx*ny; ++index){
    if(node_types[index] == NodeType::boundary){
